...
4. **小智语音固件** (`wall-e_xiaozhi/`)：**当前主线**。单块 ESP32-S3 运行 vendored 小智 ESP-IDF 固件（上游 78/xiaozhi-esp32 @ e0074e9），板型代码在 `main/boards/walle/`：唤醒词 + 云端 LLM + MCP 动作工具 + 运动核心 + 双屏 + Web 控制面板（:80，API 兼容 Flask 版）+ USB 串口协议。迁移细节见 `docs/XIAOZHI_MIGRATION.md`。
