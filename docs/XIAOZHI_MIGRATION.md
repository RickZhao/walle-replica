...
> 本文档记录 Wall-E 机器人从 Arduino 固件迁移到小智 ESP-IDF 语音方案的全部信息：源码溯源、构建烧录、引脚、MCP 工具清单、控制面架构。
>
...
ST7789 1.3" 240x240，与 GC9A01 共用 SPI3（CS=42/DC=14，RST/BL 共接）。通过 `lvgl_port_add_disp` 注册为 esp_lvgl_port 任务上的**第二个 LVGL 屏**（须在主屏初始化之后调用 `Init()`）。内容 1s 刷新：电量、Wi-Fi IP、助手状态、自动模式。引脚与偏移在 `config.h` 的 `STATUS_DISPLAY_*`；画面偏移时调 `STATUS_DISPLAY_OFFSET_X/Y`。
